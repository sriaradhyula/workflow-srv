...
from agent_workflow_server.generated.models.value_run_error_update import ValueRunErrorUpdate
from agent_workflow_server.generated.models.value_run_interrupt_update import ValueRunInterruptUpdate
...
STREAMEVENTPAYLOAD_ONE_OF_SCHEMAS = ["CustomRunResultUpdate", "ValueRunErrorUpdate", "ValueRunInterruptUpdate", "ValueRunResultUpdate"]
...
class StreamEventPayload(BaseModel):
    ...
    actual_instance: Optional[Union[CustomRunResultUpdate, ValueRunErrorUpdate, ValueRunInterruptUpdate, ValueRunResultUpdate]] = None
    one_of_schemas: List[str] = ["CustomRunResultUpdate", "ValueRunErrorUpdate", "ValueRunInterruptUpdate", "ValueRunResultUpdate"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, ValueRunInterruptUpdate):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ValueRunInterruptUpdate`")
        else:
            match += 1
        # validate data type: ValueRunErrorUpdate
        if not isinstance(v, ValueRunErrorUpdate):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ValueRunErrorUpdate`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in StreamEventPayload with oneOf schemas: CustomRunResultUpdate, ValueRunErrorUpdate, ValueRunInterruptUpdate, ValueRunResultUpdate. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in StreamEventPayload with oneOf schemas: CustomRunResultUpdate, ValueRunErrorUpdate, ValueRunInterruptUpdate, ValueRunResultUpdate. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into StreamEventPayload with oneOf schemas: CustomRunResultUpdate, ValueRunErrorUpdate, ValueRunInterruptUpdate, ValueRunResultUpdate. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into StreamEventPayload with oneOf schemas: CustomRunResultUpdate, ValueRunErrorUpdate, ValueRunInterruptUpdate, ValueRunResultUpdate. Details: " + ", ".join(error_messages))
        else:
            return instance
